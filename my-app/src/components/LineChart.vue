  ...
  mounted() {
    ...
    d3.json("/data_unemployed.json")
      .then(data => {
        ...
        chart.selectAll(".lines path").on("mouseover", handleMouseOver);
        ...
        d3.select(".chart").on("dblclick", function() {
          let date = xScale.invert(event.clientX),
            ...
            month =
              yearMax === 2013 && date.getMonth() > 8 ? 8 : date.getMonth(),
          ...
          this.newData = updateData(data, dateMin, dateMax);
          console.log("newData", this.newData);
          zoom = zoom ? 0 : 1;

          function setScale2(newData) {
            console.log("newData in setScale2", newData);
            xScale
          ...
          function updateLines(newData, newDates) {
            var t = d3.transition().duration(500);

            var line = d3
              .line()
              .defined(d => !isNaN(d))
              .x((d, i) => xScale(newDates[i]))
              .y(d => yScale(d));
          ...
          if (zoom) setScale2(this.newData);
          else setScale1();
          updateAxis();
          if (zoom) updateLines(this.newData, this.newData.dates);
          else updateLines(data, dates);
        });
        ...
        function moved() {
          let mvdates = zoom ? this.newData.dates : dates;
          let mvdata = zoom ? this.newData : data;
          d3.event.preventDefault();
          const ym = yScale.invert(event.offsetY);
          const xm = xScale.invert(event.offsetX);
          const i1 = d3.bisectLeft(mvdates, xm, 1);
          const i0 = i1 - 1;
          const i = xm - mvdates[i0] > mvdates[i1] - xm ? i1 : i0;
          const s = mvdata.series.reduce((a, b) =>
          ...
          dot
            .attr(
              "transform",
              `translate(${xScale(mvdates[i])},${yScale(s.values[i])})`
            )
            .raise();
          dot
            .select("text")
        ...
        function updateData(data, dateMin, dateMax) {
          let newDates = data.dates.filter(
              date => date >= dateMin && date <= dateMax
            ),
